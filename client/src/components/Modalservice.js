  ...
  return (
   ...
   <Modal show={show} onHide={handleClose} size='lg' >
      <Modal.Header  style={{background: "rgb(255,241,248)"}}  closeButton>
        <Modal.Title style={{fontFamily:"cursive", marginLeft:"30px"}} >  <h3> {service?.name}</h3> 
        </Modal.Title>
      </Modal.Header>
      <Modal.Body style={{display:"flex",background: "rgb(255,241,248)"}}>
        <div>
          <img src={service?.img} style={{width:"800px", height:"380px", boxShadow:"5px 1px 46px 3px rgba(53,45,53,0.53)", marginLeft:"20px",marginRight:"200px"}} />
        </div>
...
<p style={{ fontFamily:"serif", fontSize:"15px", textAlign:"center", marginLeft:"10px", marginRight:"30px"}}>
 ...
 Beauty has a purpose, and purpose is you !</p>
          ...
          <p style={{fontFamily:"cursive", paddingBottom:"10px"}}>{service?.description}</p>
          <hr/>
...
<input type="tel" placeholder="Number phone" onChange={(e)=>setnewreservations({...newreservations,phone:e.target.value})}/>
      ...
      <Modal.Footer style={{background: "rgb(255,241,248)"}}>
        ...
        <Button style={{backgroundColor:"pink", border:"none"}} onClick={()=>{dispatch(addreservation(newreservations));Swal.fire({
  title: "Reserved!",
